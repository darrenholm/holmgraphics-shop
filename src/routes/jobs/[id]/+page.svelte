...
// Label printing
  ...
  // ─── Round 2 edit state ───────────────────────────────────────────
  // Kept flat (one set of vars per tab) instead of nesting in an object
  // because Svelte 4 reactivity is cheaper with primitives.

  // WiFi: adding/editing single row. editingWifiId=null → not editing.
  ...
  // Modules: adding/editing single row + sign→module picker.
  ...
  // Picker: which module row the user is currently picking a sign for.
  let pickingSignForModuleId = null;

  // LED Signs: inline edit + add + service-log state.
  let addingSign = false;
  let newSign = { sign_name: '', location: '' };
  let savingSign = false;
  let editingSignId = null;
  let editSign = {};                // full field set; populated from the row
  let savingEditSign = false;
  // Service log form: signId currently being logged against + form values.
  ...
  async function submitNewWifi() {
    if (!newWifi.location && !newWifi.ssid) {
      alert('Enter at least a location or SSID.');
      return;
    }
    savingWifi = true;
    try {
      const created = await api.createWifi(project.client_id, newWifi);
  ...
  async function submitNewModule() {
    if (!newModule.module_id_no.trim()) {
    ...
    try {
      ...
      addingModule = false;
      // A newly-created module won't appear in the per-client list until
      // a sign is linked to it. Refresh anyway in case the sign-link step
      // follows immediately.
  ...
  async function saveEditModule() {
    ...
    try {
      ...
      // Preserve the nested `signs` array we already have since the PUT
      // response only includes the module row itself.
      modules = modules.map(m => m.id === updated.id
        ? { ...m, ...updated }
        : m);
  ...
  async function deleteModuleRow(m) {
    if (!confirm(`Delete module "${m.module_id_no}"? Linked signs will stay but become unlinked.`)) return;
    try {
  ...
  // Sign→module picker: show a list of this client's signs with a "Link"
  // button each. Once linked, refresh the modules tab so the sign shows
  // up in the "Used by" column.
  ...
  async function submitNewSign() {
    ...
    savingSign = true;
    try {
      const created = await api.createLedSign(project.client_id, newSign);
      // Seed the service_history so the template's .length check doesn't blow up.
  ...
  function startEditSign(s) {
    editingSignId = s.id;
    // Copy all writable fields so the form can bind to them. install_date
    // from the API is an ISO datetime; the <input type="date"> needs YYYY-MM-DD.
    editSign = {
  ...
  async function saveEditSign() {
    savingEditSign = true;
    try {
      // Coerce numeric-ish fields that arrived as strings from <input>.
      ...
      ledSigns = ledSigns.map(s => s.id === updated.id
        ? { ...s, ...updated }   // preserve service_history
        : s);
      editingSignId = null;
    } catch (e) { alert(e.message); }
  ...
  async function deleteSign(s) {
    if (!confirm(`Delete sign "${s.sign_name || s.id}" and all its service history?`)) return;
    try {
  ...
  function startLogService(signId) {
    ...
    if (!expandedSignId || expandedSignId !== signId) expandedSignId = signId;
  }
  ...
  async function submitService() {
    ...
    savingService = true;
    try {
      const created = await api.logLedService(loggingServiceForSignId, newService);
      // Splice into the right sign's service_history so the UI updates
      // without a round-trip reload. The employee name isn't on `created`
      // (API returns the raw row) so reload the signs to pick it up.
        ...
        {#if addingSign}
          <div class="inline-form">
            ...
            <div class="form-grid">
              ...
              <label>Location<input bind:value={newSign.location} placeholder="Front of building" /></label>
            </div>
            <p class="hint">Spec details (pitch, dimensions, serial, etc.) can be filled in after the sign is created — use <em>Edit</em> on the row.</p>
              ...
              <div class="ref-card">
                {#if editingSignId === s.id}
                  ...
                  <div class="form-grid">
                    ...
                    <label>Pitch<input bind:value={editSign.pitch} placeholder="e.g. 10" /></label>
                    ...
                    <label>Module size<input bind:value={editSign.module_size} placeholder="e.g. 320×160" /></label>
        ...
        {#if addingWifi}
          <div class="inline-form">
            ...
            <div class="form-grid">
              ...
              <label>SSID<input bind:value={newWifi.ssid} placeholder="NetworkName" /></label>
              <label class="span-2">Password<input bind:value={newWifi.password} placeholder="secret" /></label>
            </div>
            <div class="form-actions">
      ...
      <div class="card">
        ...
        {#if addingModule}
          <div class="inline-form">
            ...
            <p class="hint">
              Modules live in a shared inventory across all clients. After creating, link it to one of this client's LED signs using the <em>Link sign</em> button on the new row.
            </p>
            <div class="form-grid">
              <label>Module ID # *<input class="mono" bind:value={newModule.module_id_no} placeholder="e.g. P10-RGB-v2" /></label>
        ...
        {:else if modules.length === 0 && !addingModule}
          <p class="empty-msg">
            ...
            Create one above, or from an existing sign click <em>Edit</em> and set the module link.
          </p>
              ...
              {#each modules as m}
                {#if editingModuleId === m.id}
                  <tr>
                    ...
                    <td><input type="number" min="0" bind:value={editModule.starting_inventory} style="text-align:right" /></td>
                      ...
                      {:else}
                        {#each m.signs as sg, i}
                          <span>
                            ...
                            {#if $isStaff}
                              <button class="btn-link-danger" title="Unlink this sign" on:click={() => unlinkSignFromModule(sg.id)}>✕</button>
                            {/if}
                      ...
                      <td colspan={$isStaff ? 5 : 4}>
                        <div class="inline-form inline-form-nested">
                          ...
                          {:else if ledSigns.length === 0}
                            <p class="hint">No LED signs on file for this client yet.</p>
                          {:else}
